...
public class RegistroHuesped extends JFrame {
	...
	private void guardarHuesped() {
		...
		Date fechaEscogida = txtFechaN.getDate();
		...
		String telefono=txtTelefono.getText();
			...
			Integer menorEdad = Math.round((dateHoy.getTime()- fechaEscogida.getTime()) / 31557600000L);
			...
			if(!comprobarNumeroDeTelefono(telefono)) {
				JOptionPane.showMessageDialog(null, "Debes llenar el campo\nteléfono solo con números\n y minimo 8 numeros.");
				return;
			}
			if(huespedesController.guardar(huespedes)) {
				Exito exito = new Exito();
				exito.setVisible(true);	
				dispose();
			}
	}
	public boolean comprobarNumeroDeTelefono(String s) {
		if(s.length()<8){
			return false;
		}
